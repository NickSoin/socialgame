...
export const signInWithProviderAction = actionClient
  ...
  .action(async ({ parsedInput: { provider, next } }) => {
    ...
    const requestOrigin = requestHeaders.get("origin") ?? toSiteURL("/");
    const redirectToURL = new URL("/auth/callback", requestOrigin);
    if (next) {
      redirectToURL.searchParams.set("next", sanitizeAuthRedirect(next));
    }
