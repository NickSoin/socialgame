  ...
  return <div className="ra-app">
    ...
    <main className="ra-main">
      ...
      <section className="ra-panel">
        ...
        <div className="ra-users-table"><div className="gm-table-head"><span>User</span><span>Email</span><span>Role</span><span>Status</span><span>Last sign in</span><span>Actions</span></div>{data.users.map((user) => <div className="gm-table-row" key={user.id}><span><strong>{user.displayName ?? 'No public name'}</strong><small>@{user.username ?? 'not-set'}</small></span><span>{user.email}</span><span><b className={`ra-role ra-role--${user.role}`}>{user.role}</b></span><span>{user.verified ? 'Verified' : 'Unverified'}</span><span>{formatDate(user.lastSignInAt)}</span><span>{user.role === 'root' ? <small>Environment controlled</small> : user.role === 'game_designer' ? <button className="ra-revoke" disabled={isPending} type="button" onClick={() => window.confirm(`Revoke Game Designer from ${user.email}?`) && mutate({ action: 'revoke', userId: user.id }, 'Role revoked immediately.')}><UserMinus size={14} />Revoke</button> : <button disabled={isPending || !user.verified} type="button" onClick={() => window.confirm(`Grant Game Designer to ${user.email}?`) && mutate({ action: 'grant', email: user.email }, 'Role granted immediately.')}><UserPlus size={14} />Grant</button>}</span></div>)}</div>
      </section>
      ...
      <div className="ra-grid">
        <section className="ra-panel">
          ...
          <form className="ra-grant-form" onSubmit={(event) => { event.preventDefault(); if (window.confirm(`Grant Game Designer to ${grantEmail}?`)) mutate({ action: 'grant', email: grantEmail }, 'Role or pending assignment created.'); }}><input required type="email" placeholder="designer@example.com" value={grantEmail} onChange={(event) => setGrantEmail(event.target.value)} /><button disabled={isPending} type="submit"><UserPlus size={15} />Grant Game Designer</button></form>
          <div className="ra-pending"><h3>Pending assignments</h3>{data.pending.filter((item) => item.status === 'pending').length ? data.pending.filter((item) => item.status === 'pending').map((item) => <article key={item.id}><div><strong>{item.email}</strong><small>Requested {formatDate(item.requested_at)}</small></div><span>{item.role}</span><button className="ra-revoke" disabled={isPending} type="button" onClick={() => window.confirm(`Revoke pending assignment for ${item.email}?`) && mutate({ action: 'revoke_pending', assignmentId: item.id }, 'Pending assignment revoked.')}><UserMinus size={14} />Revoke</button></article>) : <p>No pending assignments.</p>}</div>
        </section>
