...
import { optimizeSteamScreenshot, STEAM_SCREENSHOT_MAX_BYTES } from "./lib/optimize-steam-screenshot.mjs";
...
if (options.dryRun) {
  let query = supabase.from("steam_games")
...
await ensureBucket();
const { data: run, error: runError } = await supabase.from("steam_enrichment_runs")
  .insert({ worker_id: workerId, status: "running" }).select("id").single();
...
try {
  ...
  const { data: quality, error: qualityError } = await supabase.rpc("get_steam_game_data_quality_report");
  if (qualityError) throw qualityError;
  summary.stillPending = Number(quality?.[0]?.media_pending_count ?? 0);
  const status = summary.failed ? (summary.succeeded || summary.partial ? "partial" : "error") : "success";
  await finishRun(status);
  console.log(JSON.stringify({ runId: run.id, workerId, discovery, summary, quality: quality?.[0] ?? null }, null, 2));
} catch (error) {
...
async function invokeDiscovery() {
  const response = await fetch(new URL("/functions/v1/sync-steam-details", supabaseUrl), {
    ...
    body: JSON.stringify({ limit: options.limit, ...(options.appId ? { appId: options.appId } : {}) }),
    signal: AbortSignal.timeout(180_000),
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(`Discovery failed (${response.status}): ${payload.error ?? "unknown error"}`);
  return payload;
}
...
async function processMediaJob(job) {
  ...
  try {
    const { data: existing, error: existingError } = await supabase.from("steam_game_media")
      .select("id,position,original_source_url,storage_bucket,storage_path")
      .eq("steam_app_id", job.steam_app_id).eq("kind", "screenshot").eq("active", true);
    ...
    for (const screenshot of screenshots) {
      ...
      const { error: uploadError } = await supabase.storage.from("steam-game-media").upload(
        objectPath,
        optimized.buffer,
      ...
      const { data: previous, error: publishError } = await supabase.rpc("publish_steam_game_media", {
      ...
      if (oldPath && oldPath !== objectPath) {
        const { error: cleanupError } = await supabase.storage.from("steam-game-media").remove([oldPath]);
        if (cleanupError) console.warn("Old Steam media cleanup will be retried", { appId: job.steam_app_id, oldPath });
      }
    }
    ...
    for (const old of existing) {
      ...
      const { error } = await supabase.from("steam_game_media").update({ active: false }).eq("id", old.id);
      if (error) throw error;
      const { error: cleanupError } = await supabase.storage.from(old.storage_bucket).remove([old.storage_path]);
      if (cleanupError) console.warn("Retired Steam media cleanup will be retried", { appId: job.steam_app_id, path: old.storage_path });
    }

    const status = screenshots.length >= 2 ? "complete" : screenshots.length === 1 ? "partial" : "not_available";
    const now = new Date().toISOString();
    const { error: stateError } = await supabase.from("steam_game_enrichment_state").update({
      status,
      last_success_at: now,
      retry_after: null,
    ...
    return {
      status: status === "complete" ? "succeeded" : status === "not_available" ? "unavailable" : status,
      uploaded,
      skippedUnchanged,
    };
  } catch (error) {
...
async function downloadTrustedImage(sourceUrl) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      if (Number.isFinite(contentLength) && contentLength > 20 * 1024 * 1024) throw new Error("screenshot_too_large");
      const buffer = Buffer.from(await response.arrayBuffer());
      if (!buffer.length || buffer.length > 20 * 1024 * 1024) throw new Error("screenshot_too_large");
      return buffer;
    } catch (error) {
      lastError = error;
...
function assertTrustedImageUrl(url) {
  ...
  if (url.protocol !== "https:" || !hosts.has(url.hostname)) throw new Error("untrusted_screenshot_url");
}

function validScreenshot(value) {
  return value && (value.position === 1 || value.position === 2) && typeof value.sourceUrl === "string";
}
...
function parseArguments(arguments_) {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    else if (argument === "--limit") parsed.limit = integerOption(arguments_[++index], "--limit", 1, 200);
    else if (argument === "--app-id") parsed.appId = integerOption(arguments_[++index], "--app-id", 1, 999_999_999_999);
    else if (argument === "--concurrency") parsed.concurrency = integerOption(arguments_[++index], "--concurrency", 1, 8);
